#!/usr/bin/env python
"""
Written by Dejanira Araiza-Illan August 2016
"""

import sys
import os
import rospy
import random
import re
from std_msgs.msg import String

def main():
	rospy.init_node('human', anonymous=True) #Start node first
	rospy.loginfo("Human started")
	# random.seed(seed)
	pub = rospy.Publisher("orders",String,queue_size=1,latch=True)
	#Read the abstract test, execute it, and concretize it
	for num,command in enumerate(open(os.getcwd()+'/src/tiago_simulation/tiago_gazebo/scripts/data/cmd.txt','r')): 
# #	for num,command in enumerate(open(os.getcwd()+'/src/tiago_simulation/tiago_gazebo/scripts/mbt/'+name_file+'.txt','r')): 
# #	for num,command in enumerate(open(os.getcwd()+'/src/tiago_simulation/tiago_gazebo/scripts/abstract_tests/'+name_file+'.txt','r')):
		thecommand = re.split('\n',command)
		pub.publish(thecommand[0])
		rospy.loginfo(thecommand[0])
		rospy.sleep(1)
		# pub.publish('')
		# if thecommand[0] == 'fridge' or thecommand[0] == 'sink':
		# 	waittime = random.randint(90,120) #Concrete test: waiting intervals
		# elif thecommand[0] == 'clean':
		# 	waittime = random.randint(140,170)
		# elif thecommand[0] == 'feed':
		# 	waittime = random.randint(120,150)
		# else:
		# 	waittime = random.randint(10,20)
		# rospy.loginfo('Wait '+ str(waittime) +' secs')
		# rospy.sleep(waittime)
	rospy.loginfo('ended')
	pub.publish('ended')
	rospy.sleep(1)

#---------------------------------------------------------------------------------------------------
if __name__ == '__main__':
	try:
			main()
	except rospy.ROSInterruptException: #to stop the code when pressing Ctr+c
		pass
